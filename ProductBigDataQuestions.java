...
public class ProductBigDataQuestions {
    ...
    public static int q1() {
        ...
        // Example: Product1 contains Chemical1, Chemical2 and Product2 contains Chemical2 and Chemical3
        //          count would be 4 total chemicals
    	int numOfChem=productList.stream().mapToInt(product->product.getNumberOfChemicals()).sum();
        return numOfChem; 
    }
    public static int q2() {
        // QUESTION 2: How many different companies are in the dataset?
    	int numOfCompany=productList.stream().map(product->product.getCompany()).collect(Collectors.toSet()).size();
        return numOfCompany;
    }
    public static long q3() {
        // QUESTION 3: How many products have 4 or more chemicals?
    	long numOfChemOverFour=productList.stream().filter(product->product.getNumberOfChemicals()>=4).count();
        return numOfChemOverFour;
    }
    public static Map<String, List<Product>> q4() {
        // QUESTION 4: Create a Map with key = company name and value = list of products for that company.
        // Hint: use Collectors.groupingBy!
    	Map<String, List<Product>> productMap=productList.stream().collect(Collectors.groupingBy(product->product.getCompany()));
    	return productMap;
    }
    public static String q5(Map<String, List<Product>> companyProductMap) {
    	// QUESTION 5: Use the map you created above. Which company has the most products?
        // Hint: Use .max(Comparator) and define the Comparator to compare companies based on size of their lists.
        String mostProducies=companyProductMap.entrySet().stream().max(Comparator.comparing(Map.Entry::getValue, (p1,p2)->Integer.compare(p1.size(),p2.size()))).get().getKey();
    	return mostProducies;
    }
    public static void q6( Map<String, List<Product>> chemicalProductMap) {
        // QUESTION 6: Fill the lists (the value) of the map above.
        // Hint: Use a nested stream (one stream of productList and then a separate stream for each list of each product).
    
    /*productList.stream().forEach(product->product.getChemicals().stream()
    			.forEach(chemicalName->chemicalProductMap.entrySet().stream().filter(chemicalNamekey->chemicalName.equals(chemicalNamekey.getKey()).map(chemicalProductMap.put(chemicalName, productList)))));
   */
    	
    	productList.stream().forEach(product->product.getChemicals().stream()
    			.forEach(chemicalName->{if (chemicalProductMap.containsKey(chemicalName))
    				chemicalProductMap.get(chemicalName).add(product);}));
    }
    public static String q7() {
        // QUESTION 7: Which chemical appears in the most products?
        // Hint: use max(Comparator) again. Define your comparator to compare chemical names based on the size of the list of products.
    String mostProducies=chemicalProductMap.entrySet().stream().max(Comparator.comparing(Map.Entry::getValue, (p1,p2)->Integer.compare(p1.size(),p2.size()))).get().getKey();
