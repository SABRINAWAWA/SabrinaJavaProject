...
public class HomeworkM11Streams {
	...
	public static void main(String[] args) throws Exception {
		...
		// Extra Point
		// Q13: Find all customers who live in IL and spent more than 2000 dollars. 
		String noSpentCustomer = q13();
		System.out.println("\nQ13: Customers who live in IL and spent more than $2000: "+ noSpentCustomer);
	}
	
	public static long q1() {
		// Q1: How many customers in CA?
		long customersInCA=customerList.stream()
				.filter((Customer)->Customer.getState().equals("CA")).count();
		return customersInCA;
	}
	public static List<Customer> q2 () {
		// Q2: Create a list of all priority customers in MA.
		ArrayList<Customer> priotiryCustMA=new ArrayList<Customer>(customerList.stream()
				.filter(Customer->Customer.isPriority()==true&&Customer.getState().equals("MA"))
				.collect(Collectors.toList()));
		return priotiryCustMA;	
	}
	public static double q3() {
		// Q3: How much money have all customers spent (combined)?
		double allCustSpent=customerList.stream()
	...
	public static double q4() {
		// Q4: How much money have all priority customers spent (combined)?
		double allPriorityCustSpent=customerList.stream()
				.filter(Customer->Customer.isPriority()==true)
				.mapToDouble(Customer->Customer.getAmountSpent())
				.sum();
		return allPriorityCustSpent;
	}
	public static Map<String, Customer> q5() {
		// Q5: Create a map of all WY priority customers (key=id, value=customer)
		Map<String, Customer> mapWithWYPriCust=new HashMap<String, Customer>(customerList.stream()
				.filter(Customer->Customer.isPriority()==true&&Customer.getState().equals("WY"))
				.collect(Collectors.toMap(
						Customer->Customer.getId(),
						Customer->Customer)));
		return mapWithWYPriCust;
	}
	public static double q6() {
		// Q6: What is the greatest amount of money spent by a NY priority customer?
		double greatestAmoutMoneySpent= customerList.stream()
				.filter(Customer->Customer.isPriority()==true&&Customer.getState().equals("NY"))
				.mapToDouble(Customer->Customer.getAmountSpent())
				.max()
				.getAsDouble();
		return greatestAmoutMoneySpent;
	}
	public static String q7() {
		//Q7: Find all customers that spent > 9000.
		String customerSpent=customerList.stream()
	...
	public static void q8() {
		...
		// Print the amount spent by the customer. If there is none, nothing should be printed.
		// Note: you can test your code with a lower amount, too.
		customerList.stream().filter(Customer->Customer.getAmountSpent()>9800)
		.findAny()
		.ifPresent(System.out::println);
	}
	public static int q9(String[] numWords) {
		// Q9: Find the sum of the numbers represented in an String array.
		int sumOfNum=Arrays.stream(numWords).mapToInt(s->Integer.parseInt(s)).sum();
		return sumOfNum;
	}
	public static String q10(Integer[] nums) {
		// Q10: Create a String of the numbers represented in the array, separated by semicolons.
		String numbers=Arrays.stream(nums).map(n->String.valueOf(n)).collect(Collectors.joining(";"));
		return numbers;
	}
	...
	public static void q12(List<String> scrabbleWords) {
		...
		scrabbleWords.stream().map(number->wordScore(number)).sorted(Comparator.reverseOrder()).limit(9).forEach(number->System.out.println("\t"+number+" worth "+ number+" points"));
	}
	
	public static String q13() {
		// Find all customers who live in IL and spent more than 2000 dollars. 
		String noSpent=customerList.stream()
				.filter(Customer->Customer.getAmountSpent()>2000&&Customer.getState().equals("IL"))
